...
-- 9. EXEMPLO DE DADOS DE ACOMPANHANTES (primeiros 3 registros)
SELECT
    ...
    created_at,
    CASE
        WHEN gallery IS NOT NULL THEN array_length(gallery, 1)
        ELSE 0
    END as qtd_fotos,
    CASE
        WHEN videos IS NOT NULL THEN array_length(videos, 1)
        ELSE 0
    END as qtd_videos,
    CASE
        WHEN cities_served IS NOT NULL THEN array_length(cities_served, 1)
        ELSE 0
    END as qtd_cidades
FROM acompanhantes
